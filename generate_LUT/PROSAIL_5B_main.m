function sim_data = PROSAIL_5B_main(tts, Cab, Cw, Cm, N, LAI, psoil,LAD, VZA, RAA, skyl)
% %*************************************************************************
% %*                                                                       *
% 	main_PROSAIL
% 
...
psoil	= psoil;		% soil factor (psoil=0: wet soil / psoil=1: dry soil)
rsoil0  = psoil*Rsoil1+(1-psoil)*Rsoil2;

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%	4SAIL canopy structure parm	%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
LAI		=	LAI;%Input.lai;     	% leaf area index (m^2/m^2)
hspot	=	0.01;       % hot spot
tts		=	tts;		% solar zenith angle (?
tto		=	VZA;		% observer zenith angle (?
psi		=	RAA;         % azimuth (?
...
reflectance = rsot.*(1-skyl) + skyl .*rdot;
albedo = rsdt.*(1-skyl) + skyl .*rddt;

sim_data = [reflectance' albedo'];
% get MODIS band reflectance data
